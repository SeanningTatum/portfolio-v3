...
- **Loader not-found mapping**: `context.trpc` is the in-process caller (`createCallerFactory`), so a failed procedure throws the `TRPCError` produced by `tagToTRPC` directly (never `TRPCClientError`). A loader that fetches a single entity may catch it and translate exactly one known code into React Router's 404 convention: `if (err instanceof TRPCError && err.code === "NOT_FOUND") throw new Response("Not Found", { status: 404 }); throw err;` — rethrow everything else (root `ErrorBoundary` renders `Error` instances; DEV shows message/stack, prod stays generic). This narrow loader-boundary translation is NOT the "HTTP boundary" anti-pattern above — that rule targets raw HTTP handlers routing programs through `runProcedure` instead of `runPromiseExit`. Canonical example: `app/routes/projects/$slug.tsx`. Pair it with a route-level `ErrorBoundary` when the 404 should stay on-brand.
